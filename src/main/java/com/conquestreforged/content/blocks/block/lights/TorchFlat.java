...
public class TorchFlat extends Block implements Waterloggable {
    ...
    protected static final VoxelShape AABB = Block.createCuboidShape(4.0D, 0.0D, 4.0D, 12.0D, 16.0D, 12.0D);

    public TorchFlat(Settings properties) {
        super(properties);
        this.setDefaultState(this.stateManager.getDefaultState().with(DIRECTION, Direction.NORTH).with(LIGHT_0_3, 0).with(WATERLOGGED, false));
