...
public class DataGen {
    ...
    public static void generate(GatherDataEvent event) {
        ...
        BlockDataRegistry.getInstance().getNamespaces().forEach(namespace -> {
            ...
            event.getGenerator().addProvider((net.minecraft.data.DataProvider) new DataProviderCR(event.getGenerator(), data.build(server)));
            event.getGenerator().addProvider((net.minecraft.data.DataProvider) new DataProviderCR(event.getGenerator(), resources.build(client)));
        });
    }
}
