...
public class ItemList implements Sorter<ItemStack>, Comparator<ItemStack> {
    ...
    @Override
    public void apply(DefaultedList<ItemStack> items) {
        fill(items);
        items.sort(this);
    }

    private void fill(DefaultedList<ItemStack> items) {
    ...
    private static int getIndex(ItemStack stack, Map<String, Entry> index) {
        String name = Registry.ITEM.getId(stack.getItem()) + "";
    ...
    private static boolean contains(List<ItemStack> items, String find) {
        for (ItemStack stack : items) {
            if ((Registry.ITEM.getId(stack.getItem()) + "").equals(find)) {
                return true;
            }
        }
        return false;
