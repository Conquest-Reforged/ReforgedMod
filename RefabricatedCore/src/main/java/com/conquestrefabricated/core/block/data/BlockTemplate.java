...
public class BlockTemplate {
    ...
    public Identifier getRegistryName(BlockName name) {
        if (state == null) {
            return new Identifier(name.getNamespace(), name.format("%s", plural));
        }
        return new Identifier(name.getNamespace(), name.format(state.name(), plural));
    }

    public void addClientResources(VirtualResourcepack.Builder builder, BlockName name, Textures textures, Identifier regName) {
    ...
    public void addServerResources(VirtualResourcepack.Builder builder, BlockName name, Identifier regName) {
        addRecipe(builder, name, regName);
    }

    private void addState(VirtualResourcepack.Builder builder, BlockName name, Identifier regName) {
        if (state == null) {
        ...
        String templatePath = Locations.statePath(new Identifier(templateName));
        String virtualPath = Locations.statePath(new Identifier(virtualName));
    ...
    private void addModel(VirtualResourcepack.Builder builder, BlockName name, Textures textures, Identifier regName) {
        if (blockModels == null) {
        ...
        for (Model model : blockModels) {
            ...
            String templatePath = Locations.modelPath(new ModelIdentifier(templateName));
            String virtualPath = Locations.modelPath(new ModelIdentifier(virtualName));
    ...
    private void addItem(VirtualResourcepack.Builder builder, BlockName name, Identifier regName) {
        if (itemModel == null) {
        ...
        String templatePath = Locations.modelPath(new ModelIdentifier(templateName));
        String virtualPath = Locations.modelPath(new ModelIdentifier(itemModelName));
    ...
    private void addRecipe(VirtualResourcepack.Builder builder, BlockName name, Identifier regName) {
        ...
        String item = new Identifier(output).toString();
        Ingredient result = createIngredient(recipe.name(), item, plural);

        Ingredient[] ingredients = push(recipe.ingredients(), result);
        String templatePath = Locations.recipePath(new Identifier(templateName));
        String virtualPath = Locations.recipePath(new Identifier(recipeName));
    ...
    private JsonOverride getOverrides(BlockName name, Ingredient[] ingredients) {
        ...
        for (Ingredient ingredient : ingredients) {
            String find = new Identifier(ingredient.template()).toString();
    ...
    private String getIngredient(BlockName name, Ingredient ingredient) {
        ...
        if (Registry.ITEM.containsId(new Identifier(name.getNamespace(), itemName))) {
