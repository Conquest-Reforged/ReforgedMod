...
public class SmallDoor extends WaterloggedHorizontalDirectionalShape {
    ...
    public VoxelShape getShape(BlockState state) {
        if (state.get(HorizontalDirectionalWaterloggedToggle2.TOGGLE) == 1) {
    ...
    public BlockState getPlacementState(ItemPlacementContext context) {
        return super.getPlacementState(context).with(HorizontalDirectionalWaterloggedToggle2.TOGGLE, 1).with(DIRECTION, context.getPlayerFacing()).with(HINGE, this.getHingeSide(context));
    }
    ...
    public ActionResult onUse(BlockState state, World level, BlockPos blockPos, PlayerEntity player, Hand hand, BlockHitResult hitResult) {
        if (!player.getAbilities().allowModifyWorld) {
            ...
            level.setBlockState(blockPos, state.cycle(HorizontalDirectionalWaterloggedToggle2.TOGGLE), 3);
